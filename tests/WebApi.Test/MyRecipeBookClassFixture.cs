...
public class MyRecipeBookClassFixture : IClassFixture<CustomWebApplicationFactory>
{
    ...
    protected async Task<HttpResponseMessage> DoPost(string method, object request, string token = "", string culture = "pt")
    {
        ChangeRequestCluture(culture);
        AuthorizeRequest(token);
        return await _httpClient.PostAsJsonAsync(method, request);
    }

    protected async Task<HttpResponseMessage> DoPostFormData(string method, object request, string token = "", string culture = "pt")
    {
        ChangeRequestCluture(culture);
        AuthorizeRequest(token);
    ...
    protected async Task<HttpResponseMessage> DoDelete(string method, string token = "", string culture = "pt")
    {
        ChangeRequestCluture(culture);
        AuthorizeRequest(token);
    ...
    private void ChangeRequestCluture(string culture)
    {
    ...
    private void AddListToMultipartContent(MultipartFormDataContent multipartContent, string propertyName, IList list)
    {
    ...
    private void AddClassListToMultipartContent(MultipartFormDataContent multipartContent, string propertyName, IList list)
    {
        var index = 0;
        foreach (var item in list)
        {
